...
class GeneratorCommand extends BaseCommand
{
    ...
    protected $fillables = [];
    protected $quantity;
    protected $presets = "../../config/crudgenerator.presets";
    ...
    public function run(array $params)
    {
        ...
        $this->fillables = $this->loadDataFromCmd();
    ...
    {
        return file_get_contents(file_exists(APPPATH("Views/stubs/$type.stub")) ? APPPATH("Views/stubs/$type.stub") : __dir__."/../stubs/$type.stub");
    }
    ...
    protected function model()
    {
		...
		(new Filesystem)->ensureDirectoryExists(APPPATH('Models'));
		
        file_put_contents(APPPATH("Models/{$this->name}.php"), $template);
    }
    ...
    protected function loadDataFromCmd() {
        $fillables = [];
        do {
            $field = CLI::prompt('Field name');
            if (strtolower($field) !== 'done' && strtolower($field) !== "stop" && $field != null) {

                $fillables[] = [
                    'field' => strtolower(str_replace(' ', '_', $field)),
                    'preset' => CLI::prompt('Choose preset?', ['text','textarea']),
                    'nullable' => CLI::prompt('Nullable?', ['Y','n'])
                ];

                $count = count($fillables);
                CLI::write("$field successfully added. Currently {$count} fields added. Type \"done\" when finished.");
            };

            if ($field == null) {
                CLI::write('Invalid field name', 'red');
            }
